...
export const NotificationList: React.FC = () => {
  ...
  const handleSelect = (n: Notification) => setSelectedId(n.id);

  if (selectedId) {
    return <NotificationDetail notificationId={selectedId} onBack={() => setSelectedId(null)} />;
  }
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in max-w-2xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-border">
        ...
        {unreadCount > 0 && (
          <Button variant="secondary" icon={<CheckCheck size={15} />} onClick={markAllAsRead}>
            Oznacz wszystkie
          </Button>
        )}
      </div>
          ...
          {notifications.map(n => (
            <button
              ...
              className={`w-full text-left flex items-start gap-4 p-4 rounded-xl border transition-all
                ...
                ${n.isRead ? 'bg-bg-sidebar border-border' : 'bg-bg-sidebar border-border ring-1 ring-primary/15'}`}
            >
